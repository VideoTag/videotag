 ...
 * 
 * @version 4.1.0 - Fixed YouTube export for local file viewing
 */
...
// ============================================
// FIXED HTML EXPORT - Same structure as main app
// ============================================
function generateHTMLContent() {
  const data = getCommentsData();
  const markers = getTimelineMarkersData();
  const videoId = state.currentVideoId;
  const isShorts = state.currentProvider === 'youtube_shorts';
  ...
  const maxWidthStyle = isShorts ? 'max-width: 400px; margin: 0 auto;' : '';
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .seek-controls span { color: #fff; font-size: 12px; white-space: nowrap; }
    .seek-controls input {
      padding: 6px 10px;
    ...
    .seek-controls input:focus { outline: none; border-color: var(--primary); }
    ...
    .timeline-marker.reaction { background: var(--yellow); box-shadow: 0 0 10px rgba(251, 191, 36, 0.5); }
    .timeline-marker.comment { background: var(--cyan); box-shadow: 0 0 10px rgba(34, 211, 238, 0.5); }
    ...
    .comment:hover { transform: translateX(4px); box-shadow: 0 4px 16px rgba(0,0,0,0.3); border-color: var(--primary); }
    ...
    .footer { text-align: center; margin-top: 3rem; padding-top: 2rem; border-top: 1px solid rgba(255,255,255,0.05); color: var(--muted); font-size: 0.875rem; }
...
<body>
      ...
      <div class="seek-controls">
        ...
        <div class="timeline" id="timeline">
          ...
          <div class="timeline-markers">
            ${markers.map(m => `<div class="timeline-marker ${m.type}" style="left: ${m.position}%" data-time="${m.timestamp}"><div class="tooltip">[${m.time}] ${sanitizeHTML(m.text)}</div></div>`).join('')}
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="stats">
    <div class="stat"><div class="stat-value">${data.filter(d => d.type === 'comment').length}</div><div class="stat-label">Comments</div></div>
    <div class="stat"><div class="stat-value">${data.filter(d => d.type === 'reaction').length}</div><div class="stat-label">Reactions</div></div>
    <div class="stat"><div class="stat-value">${formatTime(state.videoDuration)}</div><div class="stat-label">Duration</div></div>
  </div>

  <h2>Comments & Reactions</h2>
  <div class="comments-list">
    ${data.map((d) => `<div class="comment${d.type === 'reaction' ? ' reaction' : ''}" data-time="${d.timestamp}"><button class="timestamp" data-time="${d.timestamp}">[${d.time}]</button><span class="text">${d.emoji ? `<span class="emoji">${d.emoji}</span>` : ''}${sanitizeHTML(d.text)}</span></div>`).join('')}
  </div>

  <div class="footer"><p>Exported from <strong>ReactVid</strong></p></div>
    ...
    (function() {
      ...
      function seekTo(seconds) {
        ...
        if (provider === 'youtube' || provider === 'youtube_shorts') {
          newSrc = 'https://www.youtube.com/embed/' + videoId + '?rel=0&modestbranding=1&start=' + sec + '&autoplay=1';
        } else if (provider === 'vimeo') {
          newSrc = 'https://player.vimeo.com/video/' + videoId + '#t=' + sec + 's';
        } else if (provider === 'dailymotion') {
          newSrc = 'https://www.dailymotion.com/embed/video/' + videoId + '?start=' + sec + '&autoplay=1';
        } else {
          newSrc = 'https://www.youtube.com/embed/' + videoId + '?rel=0&modestbranding=1&start=' + sec + '&autoplay=1';
        }
...
<head>
  ...
  <title>${sanitizeHTML(state.videoTitle)} - ReactVid Export</title>
  <style>
    :root { --primary: #6366f1; --primary-light: #818cf8; --secondary: #ec4899; --bg: #05050a; --surface: #0f0f1a; --surface-elevated: #151522; --text: #fff; --text-secondary: #a0a0b8; --muted: #5a5a70; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Segoe UI', system-ui, sans-serif; background: var(--bg); color: var(--text); line-height: 1.6; padding: 2rem; max-width: 1000px; margin: 0 auto; }
    .header { text-align: center; margin-bottom: 2rem; }
    h1 { font-size: 2rem; margin-bottom: 0.5rem; background: linear-gradient(135deg, #6366f1, #ec4899); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
    .meta { color: var(--muted); font-size: 0.875rem; margin-bottom: 1rem; }
    .meta span { display: inline-block; padding: 0.25rem 0.75rem; background: var(--surface); border-radius: 20px; margin: 0.25rem; }
    .video-section { background: var(--surface); border-radius: 16px; padding: 1rem; margin-bottom: 2rem; box-shadow: 0 8px 32px rgba(0,0,0,0.5); }
    .video-container { background: #000; border-radius: 12px; overflow: hidden; margin-bottom: 1rem; }
    .video-container video { width: 100%; max-height: 70vh; display: block; }
    .time-display { display: flex; align-items: center; justify-content: center; gap: 1rem; padding: 0.75rem; background: var(--surface-elevated); border-radius: 8px; font-family: monospace; }
    .time-display .current { font-size: 1.5rem; font-weight: 700; color: var(--primary-light); }
    ...
    .stat { padding: 1.25rem; background: var(--surface); border-radius: 12px; text-align: center; border: 1px solid rgba(255,255,255,0.05); }
    .stat-value { font-size: 1.75rem; font-weight: 700; background: linear-gradient(135deg, var(--primary), var(--secondary)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
    .stat-label { font-size: 0.75rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em; }
    h2 { font-size: 1.25rem; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem; }
    h2::before { content: ''; display: block; width: 4px; height: 24px; background: var(--primary); border-radius: 2px; }
    .comments-list { display: flex; flex-direction: column; gap: 0.75rem; }
    .comment { display: flex; gap: 1rem; padding: 1rem 1.25rem; background: var(--surface); border-radius: 12px; border: 1px solid rgba(255,255,255,0.05); transition: all 0.2s; cursor: pointer; }
    .comment:hover { transform: translateX(4px); box-shadow: 0 4px 16px rgba(0,0,0,0.3); border-color: var(--primary); }
    .comment.active { border-color: var(--primary); background: var(--surface-elevated); }
    .comment.reaction { border-left: 3px solid #fbbf24; }
    .timestamp { padding: 0.35rem 0.75rem; background: linear-gradient(135deg, var(--primary), var(--secondary)); border-radius: 8px; font-family: monospace; font-size: 0.8rem; flex-shrink: 0; font-weight: 600; color: white; border: none; cursor: pointer; }
    .text { flex: 1; color: var(--text-secondary); }
    .emoji { font-size: 1.25rem; margin-right: 0.5rem; }
    .footer { text-align: center; margin-top: 3rem; padding-top: 2rem; border-top: 1px solid rgba(255,255,255,0.05); color: var(--muted); font-size: 0.875rem; }
    @media (max-width: 600px) { body { padding: 1rem; } .comment { flex-direction: column; gap: 0.5rem; } .timestamp { align-self: flex-start; } }
  </style>
</head>
<body>
  <div class="header">
    <h1>${sanitizeHTML(state.videoTitle)}</h1>
    <p class="meta"><span>📺 Local Video</span><span>📅 ${new Date().toLocaleDateString()}</span><span>💬 ${data.length} items</span></p>
  </div>
  
  <div class="video-section">
    <div class="video-container">
      <video id="video" controls><source src="${videoBase64}" type="${videoType}">Your browser does not support video.</video>
    </div>
    <div class="time-display"><span class="current" id="currentTime">0:00</span><span class="duration">/ ${formatTime(state.videoDuration)}</span></div>
  </div>
  ...
  <div class="comments-list" id="commentsList">
    ${data.map((d, i) => `<div class="comment${d.type === 'reaction' ? ' reaction' : ''}" data-time="${d.timestamp}"><button class="timestamp" onclick="seekTo(${d.timestamp})">[${d.time}]</button><span class="text">${d.emoji ? `<span class="emoji">${d.emoji}</span>` : ''}${sanitizeHTML(d.text)}</span></div>`).join('')}
  </div>
  ...
  <script>
    ...
    function formatTime(sec) { if (!sec || isNaN(sec)) return '0:00'; var h = Math.floor(sec / 3600); var m = Math.floor((sec % 3600) / 60); var s = Math.floor(sec % 60); function pad(n) { return n.toString().padStart(2, '0'); } return h > 0 ? h + ':' + pad(m) + ':' + pad(s) : m + ':' + pad(s); }
    function seekTo(time) { video.currentTime = time; video.play(); video.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
    video.addEventListener('timeupdate', function() { currentTimeEl.textContent = formatTime(video.currentTime); var ct = video.currentTime; comments.forEach(function(c) { var t = parseInt(c.dataset.time); if (ct >= t && ct < t + 3) { c.classList.add('active'); } else { c.classList.remove('active'); } }); });
    comments.forEach(function(c) { c.addEventListener('click', function() { seekTo(parseInt(this.dataset.time)); }); });
  </script>
</body>
</html>`;
}
...
async function exportZIP() {
  const data = getCommentsData();
  if (data.length === 0) { showToast('No comments to export', 'error'); return; }
  ...
  try {
    const zip = new JSZip();
    const folderName = sanitizeFileName(state.videoTitle) || 'export';
    const folder = zip.folder(folderName);
    
    folder.file('index.html', generateHTMLContent());
    folder.file('data.json', JSON.stringify({ video: { title: state.videoTitle, provider: state.currentProvider, id: state.currentVideoId }, comments: data, transcript: transcriptionState.transcript }, null, 2));
    
    let csv = 'timestamp,time,type,emoji,text\n';
    data.forEach(d => { csv += `${d.timestamp},"${d.time}","${d.type}","${d.emoji || ''}","${d.text.replace(/"/g, '""')}"\n`; });
...
function handleFileSelect(e) {
  ...
  if (file.size > CONFIG.MAX_FILE_SIZE) { showToast('File size exceeds 100MB limit', 'error'); return; }
...
function initEventListeners() {
  elements.videoLink?.addEventListener('input', (e) => { updateDetectedPlatform(e.target.value); });
  ...
  elements.loadVideoBtn?.addEventListener('click', () => {
    ...
    const platform = detectPlatform(url);
    if (platform) { state.currentProvider = platform; } else { showToast('Could not detect video platform', 'error'); return; }
    const videoId = extractVideoID(url, state.currentProvider);
    if (videoId) { initializePlayer(videoId); } else { showToast('Could not extract video ID', 'error'); }
  });
  
  elements.videoLink?.addEventListener('keypress', e => { if (e.key === 'Enter') elements.loadVideoBtn?.click(); });
  
  elements.pasteBtn?.addEventListener('click', async () => {
    try { const text = await navigator.clipboard.readText(); if (elements.videoLink) { elements.videoLink.value = text; updateDetectedPlatform(text); } showToast('Pasted'); } catch { showToast('Could not access clipboard', 'error'); }
  });
  ...
  elements.changeVideoBtn?.addEventListener('click', () => {
    state.videoLoaded = false; state.ytPlayer = null; state.vimeoPlayer = null; state.videoAspectRatio = null; state.originalVideoUrl = null;
    stopTranscription(); showFeaturePanels(false);
    if (elements.videoLink) elements.videoLink.value = '';
    elements.detectedPlatform?.setAttribute('hidden', '');
    clearVideoAspectClasses(); elements.videoLink?.focus();
  });
  
  elements.emojiButtons?.forEach(btn => { btn.addEventListener('click', () => { if (!state.videoLoaded) { showToast('Please load a video first', 'error'); return; } showReactionModal(btn.dataset.emoji); }); });
  elements.addCommentBtn?.addEventListener('click', () => { if (!state.videoLoaded) { showToast('Please load a video first', 'error'); return; } showCommentModal(); });
  ...
  elements.reactionText?.addEventListener('input', () => { if (elements.charCount) { elements.charCount.textContent = elements.reactionText.value.length; } });
  ...
  elements.submitReaction?.addEventListener('click', () => {
    ...
    addComment({ text, timestamp: Math.floor(getCurrentTime()), type: 'reaction', emoji: elements.selectedEmojiInput?.value });
    hideModal(elements.reactionModal); showToast('Reaction added!');
  });
  ...
  elements.commentText?.addEventListener('input', () => { if (elements.commentCharCount) { elements.commentCharCount.textContent = elements.commentText.value.length; } });
  ...
  elements.submitComment?.addEventListener('click', () => {
    ...
    addComment({ text, timestamp, type: 'comment' });
    delete elements.commentModal?.dataset.prefillTimestamp;
    hideModal(elements.commentModal); showToast('Comment added!');
  });
  ...
  elements.commentsList?.addEventListener('click', e => {
    ...
    if (timestamp && state.videoLoaded) { const time = parseInt(timestamp.dataset.time) || 0; seekToTime(time); }
  });
  ...
  document.addEventListener('click', e => { if (!e.target.closest('.export-dropdown')) { const dropdown = elements.exportDropdown || $('.export-dropdown'); dropdown?.classList.remove('open'); } });
  
  elements.exportHTML?.addEventListener('click', () => { if (!validateExport()) return; exportHTML(); closeExportDropdown(); });
  elements.exportZIP?.addEventListener('click', () => { if (!validateExport()) return; exportZIP(); closeExportDropdown(); });
  ...
  document.addEventListener('keydown', e => { if (e.key === 'Escape') { document.querySelectorAll('.modal-overlay:not([hidden])').forEach(m => hideModal(m)); $('#manualTranscriptModal')?.remove(); closeExportDropdown(); } });
  
  elements.transcribeBtn?.addEventListener('click', () => { if (!state.videoLoaded) { showToast('Please load a video first', 'error'); return; } toggleTranscription(); });
  ...
  window.addEventListener('resize', debounce(() => { if (state.videoLoaded && state.videoAspectRatio) { } }, 250));
}

function closeExportDropdown() { const dropdown = elements.exportDropdown || $('.export-dropdown'); dropdown?.classList.remove('open'); }

function validateExport() {
  if (!state.videoLoaded) { showToast('Please load a video first', 'error'); return false; }
  if (!elements.commentsList?.children.length) { showToast('No comments to export', 'error'); return false; }
  return true;
}
...
function init() {
  ...
  if (isLocalFile()) { console.log('Running from file:// - some features may be limited'); }
  
  const params = new URLSearchParams(location.search);
  const videoUrl = params.get('url') || params.get('v');
  
  if (videoUrl) {
    if (elements.videoLink) { elements.videoLink.value = videoUrl.includes('://') ? videoUrl : `https://youtu.be/${videoUrl}`; updateDetectedPlatform(elements.videoLink.value); }
    setTimeout(() => elements.loadVideoBtn?.click(), 500);
  }
}

window.addEventListener('error', (e) => { if (e.message && (e.message.includes('MetaMask') || e.message.includes('extension'))) { e.preventDefault(); return true; } });
window.addEventListener('unhandledrejection', (e) => { if (e.reason && e.reason.message && (e.reason.message.includes('MetaMask') || e.reason.message.includes('extension'))) { e.preventDefault(); return true; } });
